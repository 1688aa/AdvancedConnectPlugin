...
- 配置主选项
Settings（设置）
Connection method field（连接方法字段）输入：连接方法
Connection options field（连接选项字段）输入：连接选项
Enable built in RDP suppot（启用内置的RDP支持）
Connection address field（连接地址字段）输入：连接地址
Connection Method（连接方法）输入：rdp
